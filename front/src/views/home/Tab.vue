  ...
  >
    ...
    <el-tab-pane v-for="item in mainTabs" :key="item.name" :label="item.name" :name="item.name">
      <el-card class="card" shadow="hover">
        <!-- 以http或者https开头的地址，均使用iframe进行展示 -->
        <iframe
    ...
    <el-dropdown class="dropdown-tool" :show-timeout="0">
      ...
      <el-dropdown-menu slot="dropdown">
        <el-dropdown-item @click.native="closeCurrentTabsHandle">{{this.$t("tab.closeCurrentTabs")}}</el-dropdown-item>
        <el-dropdown-item @click.native="closeOtherTabsHandle">{{this.$t("tab.closeOtherTabs")}}</el-dropdown-item>
        <el-dropdown-item @click.native="closeAllTabsHandle">{{this.$t("tab.closeAllTabs")}}</el-dropdown-item>
        <el-dropdown-item
          @click.native="refreshCurrentTabs"
        >{{this.$t("tab.refreshCurrentTabs")}}</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>
  </el-tabs>
</template>
